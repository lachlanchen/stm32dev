...
/*
 * Brief, single-run 12 V RMS-equivalent overdrive for a 6 V tungsten lamp driven
 * from the measured 12.5 V supply through YYNMOS-1. The load still sees
 * 0/12.5 V pulses; 92.16% duty gives approximately 12.0 V RMS; reverse immediately.
 */
...
#define DUTY_TARGET                60397u
#define ASSUMED_SUPPLY_MV          12500u
#define EQUIVALENT_RMS_MV          12000u
                        ...
                        uint32_t *current_mA, uint32_t *power_mW)
{
    ...
    *power_mW = (*bus_mV * *current_mA) / 1000u;
    return *bus_mV <= 32000u && *current_mA <= 12000u;
}
