...
/*
 * Independent H7 smooth-color demo for two daisy-chained SK6812RGBW pixels.
...
/* HSV hue path R -> Y -> G -> C -> B, with W held at zero. */
...
static void run_hue_sweep(uint8_t reverse, uint32_t *next_frame)
{
    for (uint16_t frame = 0u; frame < FRAMES_PER_PHASE; frame++) {
        ...
        show_scheduled(hue_to_rgbw(hue), next_frame);
    }
}
...
static void run_white_crossfade(uint8_t white_rising, uint32_t *next_frame)
{
    for (uint16_t frame = 0u; frame < FRAMES_PER_PHASE; frame++) {
        ...
        if (white_rising != 0u) {
            pixel.r = (uint8_t)(CHANNEL_MAX - level);
            pixel.w = level;
        } else {
            pixel.r = level;
            pixel.w = (uint8_t)(CHANNEL_MAX - level);
        }
