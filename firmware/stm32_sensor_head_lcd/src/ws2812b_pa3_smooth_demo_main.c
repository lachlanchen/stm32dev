...
/*
 * Independent H7 smooth-color demo for two daisy-chained WS2812B RGB pixels.
 * TIM2_CH4 drives PA3/A3 at 800 kbit/s while a DWT deadline scheduler starts
 * frames at 1 kHz. The LCD/sensor app is not linked into this image.
 */
...
#define PHASE_DURATION_MS       500u
#define FRAMES_PER_PHASE        ((FRAME_RATE_HZ * PHASE_DURATION_MS) / 1000u)
#define FRAME_PERIOD_CYCLES     (CORE_CLOCK_HZ / FRAME_RATE_HZ)
#define HUE_BLUE_POSITION       1024u
...
static void encode_two_pixels(Pixel pixel)
{
    ...
    for (uint8_t i = 0u; i < PIXEL_COUNT; i++) {
        /* WS2812B consumes exactly 24 bits per pixel in G, R, B order. */
...
static uint8_t phase_level(uint16_t frame)
{
    return (uint8_t)(((uint32_t)frame * CHANNEL_MAX) /
                     (FRAMES_PER_PHASE - 1u));
}

/* HSV hue path R -> Y -> G -> C -> B. */
static Pixel hue_to_rgb(uint16_t hue)
{
    uint8_t sector = (uint8_t)(hue >> 8u);
    uint8_t offset = (uint8_t)(hue & 0xffu);
    uint8_t falling = (uint8_t)(255u - offset);
    Pixel pixel = {0u, 0u, 0u};
...
static void show_scheduled(Pixel pixel, uint32_t *next_frame)
{
    show_two(pixel);
...
static void run_hue_sweep(uint8_t reverse, uint32_t *next_frame)
{
    for (uint16_t frame = 0u; frame < FRAMES_PER_PHASE; frame++) {
        uint16_t hue = (uint16_t)(((uint32_t)frame * HUE_BLUE_POSITION) /
                                  (FRAMES_PER_PHASE - 1u));
        if (reverse != 0u) {
            hue = (uint16_t)(HUE_BLUE_POSITION - hue);
        }
        show_scheduled(hue_to_rgb(hue), next_frame);
    }
}

static void run_white_crossfade(uint8_t white_rising, uint32_t *next_frame)
{
    for (uint16_t frame = 0u; frame < FRAMES_PER_PHASE; frame++) {
        uint8_t level = phase_level(frame);
        Pixel pixel = {255u, 0u, 0u};

        if (white_rising != 0u) {
            pixel.g = level;
            pixel.b = level;
        } else {
            pixel.g = (uint8_t)(CHANNEL_MAX - level);
            pixel.b = (uint8_t)(CHANNEL_MAX - level);
        }
        show_scheduled(pixel, next_frame);
    }
}
...
int main(void)
{
    ...
    show_two((Pixel){0u, 0u, 0u});
    hold_ms(500u);
    next_frame = DWT->CYCCNT;

    while (1) {
        run_hue_sweep(0u, &next_frame);
        run_hue_sweep(1u, &next_frame);
        run_white_crossfade(1u, &next_frame);
        run_white_crossfade(0u, &next_frame);
    }
}
