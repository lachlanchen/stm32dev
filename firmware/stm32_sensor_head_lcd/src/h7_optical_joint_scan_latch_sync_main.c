...
/*
 * Independent, host-armed optical joint-scan controller.
 *
 ...
 * optical_schedule. This interface stays independent from the LCD app.
 */
                         ...
                         uint32_t pre_delay_ms, uint32_t *last_rgbw)
{
    ...
    for (uint32_t repeat = 0u; repeat < table_repeats; repeat++) {
        for (uint32_t index = 0u; index < table_count; index++) {
            ...
            /* Align PA2 with the SK6812 optical latch at the end of reset. */
            lamp_set(tungsten);
            sample++;
