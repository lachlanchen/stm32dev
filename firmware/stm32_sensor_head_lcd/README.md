...
identifies R/G/B/W and then ramps each die from dim to maximum. The ramp uses a
16-bit target with temporal error accumulation over the SK6812 native 8-bit
channel protocol, and leaves the pixel off afterward. It accepts `R`, `G`, `B`,
